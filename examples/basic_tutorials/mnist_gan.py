...
class WithLossD(Module):
    ...
    def forward(self, real_data, g_data):
        ...
        fake = tlx.convert_to_tensor(np.zeros(logits_fake.shape), dtype=tlx.float32).to(logits_fake.device)
...
for epoch in range(n_epoch):
    ...
    fake_image = G(tlx.convert_to_tensor(np.random.random(size=(36, 100)), dtype=tlx.float32).to(device)).cpu()
